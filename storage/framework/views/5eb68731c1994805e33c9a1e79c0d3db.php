...
<?php $__env->startSection('header_title', 'Manajemen Siswa'); ?>
<?php $__env->startSection('header_subtitle', 'Kelola data siswa aktif dan alumni'); ?>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form action="<?php echo e(route('admin.siswa.index')); ?>" method="GET" class="w-full md:w-1/3">
            <div class="relative">
                <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari NISN atau Nama..." 
        ...
        <button onclick="openSiswaModal('tambah')" class="bg-blue-200 hover:bg-blue-300 text-gray px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-sm">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4 text-gray-500"><?php echo e($siswa->firstItem() + $index); ?></td>
                    <td class="px-6 py-4 font-medium text-gray-800"><?php echo e($item->nisn); ?></td>
                    <td class="px-6 py-4 text-gray-600"><?php echo e($item->nama); ?></td>
                    <td class="px-6 py-4 text-center">
                        <span class="px-2 py-1 rounded text-xs <?php echo e($item->kelas ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400 italic'); ?>">
                            <?php echo e($item->kelas->nama_kelas ?? 'Tanpa Kelas'); ?>

                        </span>
                    </td>
                    <td class="px-6 py-4 text-gray-600"><?php echo e($item->tahunAjaran->tahun ?? '-'); ?></td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-3">
                            <button onclick="openEditSiswaModal(<?php echo e(json_encode($item)); ?>)" class="text-blue-500 hover:text-blue-700 transition-colors">
                                <i class="fa-regular fa-pen-to-square text-lg"></i>
                            </button>
                            <form action="<?php echo e(route('admin.siswa.destroy', $item->id)); ?>" method="POST" class="inline-block">
    ...
    <div class="p-5 border-t border-gray-50"><?php echo e($siswa->links()); ?></div>
</div>
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-8 transform scale-95 transition-transform duration-300" id="modal-siswa-content">
        ...
        <form id="form-siswa" method="POST">
            ...
            <div class="flex justify-end gap-3 mt-10">
                ...
                <button type="submit" class="px-8 py-2.5 bg-blue-200 hover:bg-blue-300 text-gray rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-blue-100">Simpan Data</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function openSiswaModal(type) {
        ...
        if (type === 'tambah') {
            ...
            form.action = "<?php echo e(route('admin.siswa.store')); ?>";
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\prestasi-siswa\resources\views/admin/siswa/index.blade.php ENDPATH**/ ?>